...
### 문제풀이 실패 지점 (1시간 소요)
=> 정답 값을 넣기 위한 임시의 리스트를 만들 때 `[[0] * col] * row`로 작성했는데 계속 리턴값이 for문 마지막 값으로만 나옴.
=> print로 디버깅 계속 하다가 문제 관련 질문 보고 list를 만들 때 문제 발생.

> 정답 값을 담기 위한 리스트를 만들 때는 for문을 이용하여 만들어주기. 
