...
### 문제풀이 실패 지점
 문제에서는 "자신보다 덩치가 큰 친구들의 수"에 따라 순위가 정해지는데 "자신보다 덩치가 큰 친구들의 수"에 따라 순위가 나열되는 줄 알아서 계속 실패했음.
 예) 자신보다 덩치가 큰 친구들의 수로 나타낸 임의의 리스트 예시 
 => [0, 2, 4, 1, 1, 0] 이면 정답 출력은 1 3 5 2 2 1
 => 계속 잘못 제출한 코드의 출력은 1 5 6 3 3 1 (동점자 순위 고려)
 => 문제 꼼꼼히 읽기.
