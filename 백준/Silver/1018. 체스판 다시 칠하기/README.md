...
### 문제풀이 실패 지점
 => 문제 해결법 구상(정답 표본을 두 타입으로 만들어 비교하여 다른 숫자들 카운트하여 최소값 구하기)하는데 시간이 꽤 걸림.  
 => 8\*8 탐색을 위한 for문에서 리스트 인덱스 오류를 해결하는데 시간을 꽤 썼음.  
 => if 문과 for 문을 중첩하여 사용하는데 원하는 값을 append하기 위해서는 어느 scope에 코드를 쓸 지 헷갈려서 시간이 꽤 걸림.  
