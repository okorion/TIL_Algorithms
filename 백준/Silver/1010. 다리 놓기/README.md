...
=> itertools의 combinations 이용했다가 시간 초과, pypy로 고쳐서 제출 했는데 메모리 초과

=> math의 factorial 써서 aCb를 구했는데 틀림.

=> 질문게시판 확인하니 (23 24) 케이스의 경우 정답이 24인데 23이 출력.

=> 큰 수로 표현될 때 16자리 이상이여서 곱하기로 표시되며 오차 발생

=> math.ceil로 올림 정답 제출했는데 틀림. round로 반올림 정답 제출하여 성공함.

</br>

> 공식을 이용하는 것이 속도가 매우 빠름. <br>
> 곱셈, 나눗셈, 큰 수를 다룰 때는 오차 생각하기.
