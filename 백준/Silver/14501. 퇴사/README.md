...
### 문제풀이 실패 지점 (1시간 소요 후 예제는 통과했는데 14%에서 실패함. 이후 구글링하여 반례 찾음.)

=> 처음 문제 보고 풀어보다가 실패 후 구글링

=> 다음 날 다시 풀어보다가 실패함. 아직은 다이나믹 프로그래밍 알고리즘 문제풀이에 생소


```
for _ in range(N-1, -1, -1):
    if _ + T_list[_] > N:
        new_list[_] = new_list[_+1]   #new_list[_] = 0 으로 씀. -> 첫 번째 t가 N을 초과할 시 new_list[0]은 0을 출력하게 돼서 오류
```
